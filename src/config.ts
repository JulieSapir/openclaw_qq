...
const NumberInputSchema = (defaultValue: number) =>
  z.preprocess((value) => {
...
export const QQConfigSchema = z
  .object({
    ...
    requireMention: BooleanInputSchema(true).describe(
      "群聊触发门槛（含命令）。true=仅在被@/回复机器人/命中关键词时触发；若同时开启 keywordOnlyTrigger，则群聊只认关键词。false=群内普通消息与命令都可能触发（容易被刷，谨慎关闭）。",
    ),
    ...
    nonAdminBlockedMessage: z
      .preprocess((value) => normalizeLooseString(value), z.string().optional().default("当前仅管理员可触发机器人。\n如需使用请联系管理员。"))
      .describe("非管理员被拦截时的提示文案。"),
    ...
    emptyReplyFallbackText: z
      .preprocess((value) => normalizeLooseString(value), z.string().optional().default("⚠️ 本轮模型返回空内容。请重试，或先执行 /newsession 后再试。"))
      .describe("空回复兜底文案。示例：⚠️ 本轮模型返回空内容，请 /newsession 后重试。"),
    ...
    showReplySessionSource: BooleanInputSchema(true).describe(
      "是否在每次面向用户的回复前附加来源会话标记，例如 `(from 会话写方案)` 或 `(from 主会话)`。默认开启；使用 /临时 功能区分上下文时尤其有用。",
    ),
    ...
    keywordOnlyTrigger: BooleanInputSchema(false).describe(
      "群聊仅关键词触发开关。开启后，@机器人/回复机器人消息不再触发；建议配合 keywordTriggers 一起使用。适合与其他机器人共用同一 QQ 账号时避免双重回复。",
    ),
    keywordTriggers: KeywordTriggersSchema.describe(
      '关键词触发（字符串）。Web表单填：小助手, 帮我；Raw JSON 填："小助手, 帮我"。当 requireMention=true 时，命中关键词可不@直接触发；当 requireMention=false 时，关键词不是必需条件；当 keywordOnlyTrigger=true 时，群聊里只有命中这些关键词才会触发。',
    ),
    allowBareGroupCommands: BooleanInputSchema(false).describe("群聊里是否允许只发 /model 这类裸 slash 指令就直接触发。默认关闭；关闭后需配合关键词触发，例如“椰子 /model”。"),
    enableDynamicModelCatalog: BooleanInputSchema(false).describe(
      "是否让本地 /model 列表主动探测各 provider 的 /models 并展示全量模型。默认关闭，保持更接近原版 OpenClaw 的 /model；如需动态聚合模型目录再手动开启。",
    ),
    enableTTS: BooleanInputSchema(false).describe("是否启用语音回复（依赖 OneBot 服务端支持）。"),
    sharedMediaHostDir: z
      .preprocess((value) => normalizeLooseString(value), z.string().optional().default(""))
      .describe("可选：宿主机共享媒体目录（供 NapCat 容器访问）。示例：/Users/xxx/openclaw_qq/deploy/napcat/shared_media。"),
    sharedMediaContainerDir: z
      .preprocess((value) => normalizeLooseString(value), z.string().optional().default("/openclaw_media"))
      .describe("可选：共享目录在 NapCat 容器内的挂载路径。默认 /openclaw_media。"),
    ...
    cacheInboundImagesToLocal: BooleanInputSchema(true).describe(
      "是否将当前消息以及引用/转发上下文里识别到的图片缓存到本地 MediaPaths。默认开启，便于 ACP 与多模态 agent 实际读图；关闭后仅保留 URL 提示，部分 agent 可能只能看到文字。",
    ),
    ...
    blockStreamingBreak: z
      .preprocess((value) => normalizeLooseString(value)?.toLowerCase(), z.enum(["text_end", "message_end"]).optional().default("message_end"))
      .describe("分块发送的边界。默认 message_end：等单条 assistant message 完整生成后再发，更适合 QQ 群聊。"),
