...
import { createQQSendMessageTool, createQQGetContextTool, createQQForwardMessageTool, createQQRecallMessageTool, createQQBatchRecallMessagesTool } from "./src/tools.js";
...
const plugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    // 注册主动能力 Tool —— 让 Agent 可以主动发消息、查上下文、转发消息、撤回消息
    ...
    api.registerTool((ctx) => createQQRecallMessageTool(ctx), { name: "qq_recall_message" });
    api.registerTool((ctx) => createQQBatchRecallMessagesTool(ctx), { name: "qq_batch_recall_messages" });
  },
};
